#!/usr/bin/env python3
"""Render a complete layered TamaHermes catalog from character profiles."""
    ...
    for path_value in paths:
        ...
        profiles.append(
            Profile(
                ...
                description=data.get("description", f"{data['id']} TamaHermes companion."),
...
def build_evolution_manifest(
    ...
    manifest = {
        ...
        "version": milestone,
        "contract": "tamahermes-pawn-evolution-v1",
        "generator": {
            "name": "tamahermes_gen",
...
<head>
  ...
  <title>TamaHermes {milestone} Catalog</title>
  <style>
...
def render(milestone: str, output_dir: Path, profiles: list[Profile], asset_version: str = "m2") -> dict[str, Any]:
    ...
    manifest = {
        "id": f"tamahermes-{milestone.lower()}",
        "version": milestone,
        "contract": (
            "layered-tamahermes-v1.catalog-v2.evolution-v1.generator-v4.palette-m21.lcd-m6.art-m61"
            if asset_version == "m2.1"
            else "layered-tamahermes-v1.catalog-v2.evolution-v1.generator-v1"
        ),
        "generator": {
            "name": "tamahermes_gen",
            "assetVersion": asset_version,
            ...
            "artDirection": ART_DIRECTION_ID if asset_version == "m2.1" else None,
            "profileSources": [profile.source_path for profile in profiles],
            "renderScript": "tamahermes_gen/scripts/render_catalog.py",
        },
        "cell": list(CELL),
        "screen": SCREEN,
        "lcdRuleset": LCD_RULESET_ID if asset_version == "m2.1" else None,
