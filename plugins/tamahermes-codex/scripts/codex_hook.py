...
from tamahermes.bridge import apply_bridge_event  # noqa: E402
from tamahermes.catalog import load_catalog  # noqa: E402
from tamahermes.overlay_supervisor import ensure_overlay_supervisor  # noqa: E402
from tamahermes.paths import codex_home, default_state_path, now_iso  # noqa: E402
from tamahermes.watcher import refresh_if_needed  # noqa: E402

HOOK_SCHEMA = "tamahermes.codex_plugin_hook.v1"
...
def hook_state_path(home: Path) -> Path:
    return home / "tamahermes" / "plugin-hook-state.json"
...
def overlay_supervisor_disabled() -> bool:
    return os.environ.get("TAMAHERMES_DISABLE_OVERLAY_SUPERVISOR", "").lower() in {"1", "true", "yes"}
...
def bridge_record(event: str, data: dict[str, Any], tool_name: str, turn: str) -> dict[str, Any]:
    ...
    return {
        "schema": "tamahermes.bridge.event.v1",
...
def main() -> int:
    ...
    catalog = load_catalog(ROOT, os.environ.get("TAMAHERMES_CATALOG_DIR"))
    state_path = default_state_path(home)
    had_state_before_hook = state_path.exists()
    line_id = os.environ.get("TAMAHERMES_LINE", "toast")
    machine_id = os.environ.get("TAMAHERMES_MACHINE", "aurora")
    tool_name = normalize_tool_name(data)
    turn = turn_id(data)
    ...
    if had_state_before_hook:
        try:
            ...
            print(f"[tamahermes hook refresh skipped] {exc}", file=sys.stderr)
    if not overlay_supervisor_disabled():
        try:
            ensure_overlay_supervisor(home, root=ROOT)
        except Exception as exc:  # noqa: BLE001
            print(f"[tamahermes overlay supervisor skipped] {exc}", file=sys.stderr)
...
if __name__ == "__main__":
    try:
        ...
        print(f"[tamahermes hook skipped] {exc}", file=sys.stderr)
