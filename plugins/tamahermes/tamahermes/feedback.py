...
def form_display_name(form_id: str | None) -> str:
    text = (form_id or "TamaHermes").replace("_", " ").strip()
    return text or "TamaHermes"
...
def request_avatar_reload(home: Path, delay_seconds: float = 0.25) -> dict[str, Any]:
    ...
    if not is_tamahermes_selected(global_state):
        return {"ok": True, "attempted": False, "reason": "tamahermes-not-selected"}
    ...
    set_selected_avatar_id(restored, TAMAHERMES_AVATAR_ID)
...
def queue_evolution_announcement(
    ...
    state["evolutionAnnouncement"] = {
        "schema": "tamahermes.evolution_announcement.v1",
        "formId": form_id,
...
def apply_evolution_feedback(
    ...
    reload_report = request_avatar_reload(home) if selected else {"ok": True, "attempted": False, "reason": "tamahermes-not-selected"}
