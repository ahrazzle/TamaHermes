"""tamacodex-hermes — a Hermes Agent plugin that grows a Tamacodex pet.

Hermes renders pets from ``<HERMES_HOME>/pets/<slug>/`` (``pet.json`` + an 8x9
atlas of 192x208 cells). TamaCodex already compiles exactly that atlas, so this
...
``TAMACODEX_HERMES_SYNC=1`` to run inline instead (used by the test suite).
"""
...
def _sync_mode() -> bool:
    return os.environ.get("TAMACODEX_HERMES_SYNC", "").strip().lower() in {"1", "true", "yes"}
...
def _apply(payloads: List[Dict[str, Any]]) -> None:
    ...
    from tamacodex.hermes_events import apply_hermes_hook
    from tamacodex.paths import default_state_path, hermes_home, repo_root

    root = Path(os.environ.get("TAMACODEX_REPO_ROOT") or repo_root()).expanduser().resolve()
...
def _drain() -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.debug("tamacodex-hermes: apply failed: %s", exc)
...
def record(hook_event_name: str, **payload: Any) -> None:
    ...
    if not _ensure_tamacodex_importable():
    ...
    if _sync_mode():
        try:
            ...
            logger.debug("tamacodex-hermes: sync apply failed: %s", exc)
        return
    global _worker
    with _queue_lock:
        _queue.append(entry)
        if _worker is None:
            _worker = threading.Thread(target=_drain, name="tamacodex-hermes", daemon=True)
            _worker.start()
...
def register(ctx: Any) -> None:
    """Register the TamaCodex growth hooks with Hermes."""
