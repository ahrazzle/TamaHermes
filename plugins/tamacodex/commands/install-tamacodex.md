---
description: Install or switch the Tamacodex Codex custom pet and enable hook-driven growth.
argument-hint: [toast|mais] [aurora|pulse]
---

# /install-tamacodex

Install Tamacodex into the current Codex home as a custom pet, then verify the
...
```bash
python -m tamacodex setup --line <line> --machine <machine> --force --json
```

4. Confirm that `${CODEX_HOME:-~/.codex}/pets/tamacodex/pet.json` and
...
6. Tell the user to select custom pet `Tamacodex` in Codex App settings.

The Tamacodex plugin hook updates growth during Codex tool use. The hook is
...
the mouse remains over the Tamacodex mascot for 1 second, then shows real
