...
LAUNCH_AGENT_LABEL = "com.autoark.tamacodex.overlay-supervisor"
...
def launch_agent_plist(home: Path, root: Path | None = None, python: str | None = None, interval: float = 1.0) -> dict[str, Any]:
    log_dir = Path.home() / "Library" / "Logs" / "Tamacodex"
    program_arguments = [
        python or sys.executable,
        "-m",
        "tamacodex.overlay_supervisor",
        "supervisor",
        "--codex-home",
        str(home),
        "--interval",
...
def write_launch_agent(home: Path, root: Path | None = None, python: str | None = None, interval: float = 1.0, path: Path | None = None) -> Path:
    ...
    (Path.home() / "Library" / "Logs" / "Tamacodex").mkdir(parents=True, exist_ok=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tamacodex.overlay_supervisor")
