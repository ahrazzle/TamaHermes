...
def overlay_state_path(home: Path) -> Path:
    return home / "tamacodex" / "overlay-state.json"


def overlay_pid_path(home: Path) -> Path:
    return home / "tamacodex" / "overlay-sidecar.pid"


def supervisor_pid_path(home: Path) -> Path:
    return home / "tamacodex" / "overlay-supervisor.pid"
...
def is_tamacodex_selected(global_state: dict[str, Any]) -> bool:
    return selected_avatar_id(global_state) == TAMACODEX_AVATAR_ID
...
def status_snapshot(state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        visual = {}
    return {
        "displayName": state.get("displayName") or "Tamacodex",
