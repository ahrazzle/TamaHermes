...
def native_overlay_paths(home: Path) -> dict[str, Path]:
    root = home / "tamacodex" / "native-overlay"
    return {
        "root": root,
        "binary": root / "TamacodexOverlay",
        "stamp": root / "TamacodexOverlay.sha256",
...
def codex_events_cursor_path(home: Path) -> Path:
    return home / "tamacodex" / "codex-events-cursor.json"
...
def native_overlay_source() -> Path:
    return Path(__file__).resolve().parent / "native_overlay" / "TamacodexOverlay.swift"
...
<body>
  <main class="wrap" aria-label="Tamacodex status">
    <section class="lcd">
...
def write_native_overlay_config(
    ...
    payload = {
        "schema": "tamacodex.native_overlay.config.v1",
        "visible": visible,
        "x": frame.get("x"),
        "y": frame.get("y"),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tamacodex.overlay")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.tk:
        ...
        return
    try:
        app = TamacodexOverlayApp(home, root, interval=args.interval)
    except Exception:  # noqa: BLE001
        run_headless_audio_loop(home, root, interval=args.interval)
        return
    app.run()
