...
<p align="center">
  ...
  TamaHermes uses a macOS sidecar for hover status, SFX, and supervised background growth.
</p>

<p align="center">
  <strong>Forked and ported to Hermes Agent.</strong><br>
  This fork installs TamaHermes as a Hermes pet that grows from Hermes hooks —
...
<p align="center">
  ...
  <a href="#hatch-your-own-tamahermes">Hatch Your Own</a> ·
...
The pet installs into `<HERMES_HOME>/pets/tamahermes` and grows from prompts, tool
...
TamaHermes stores numeric usage signals, not raw prompts or tool output text.
...
```text
In this repo, switch TamaHermes to the Pulse tamago shell while keeping Toast. Run ./install.sh --line toast --machine pulse, then tell me when to Wake Pet.
```
...
```text
In this repo, switch TamaHermes to the Mais companion line with the Aurora shell. Run ./install.sh --line mais --machine aurora, then tell me when to Wake Pet.
```
...
```text
In this repo, reset TamaHermes and install a fresh Toast egg in the Aurora shell. Run ./install.sh --line toast --machine aurora --reset, then tell me when to Wake Pet.
```
...
```text
In this repo, install the exact TamaHermes form toast_adult_worker with the Pulse shell. Run ./install.sh --line toast --machine pulse --form toast_adult_worker, then tell me when to Wake Pet.
```
...
```text
In this repo, list the built-in TamaHermes forms for toast and mais. Run tamahermes list-forms --line toast and tamahermes list-forms --line mais, then summarize the choices in plain English.
```
...
Each `rest` represents 10 quiet minutes: `+10 energy`, `+3 health`, `+1 mood`, `-3 restlessness`, and `quietMinutes +10`. It does not add XP and does not mean active work. TamaHermes wakes from hibernation when `energy >= 35` and `health >= 35`.
...
TamaHermes does not patch Codex App internals. It uses the custom pet package contract, local plugin hooks, local session-log adaptation, and a supervised macOS sidecar.

MIT. PRs and strange TamaHermes hatch ideas welcome.
