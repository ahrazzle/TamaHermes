...
def add_common(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--repo-root", help="TamaHermes repository root. Defaults to this source checkout.")
    ...
    parser.add_argument(
        "--petdex-home",
        help="Petdex *desktop* home to mirror the pet into (~/.petdex). Opt-in; default: TAMAHERMES_PETDEX_HOME.",
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument("--state-path", help="State ledger path. Defaults to <home>/tamahermes/state.json.")
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    if rest["applied"]:
        ...
        refresh_if_needed(catalog, state_path, home, home / "tamahermes" / "build")
        state = load_state(state_path, catalog, line_id=args.line, machine_id=args.machine)
...
def cmd_install(args: argparse.Namespace) -> None:
    ...
    build_dir = Path(args.build_dir).expanduser().resolve() if args.build_dir else home / "tamahermes" / "build"
    ...
    if resolve_target(args) == "hermes":
        print_json({"ok": True, "statePath": str(state_path), **report, "hermesPet": select_hermes_pet(state.get("petId", "tamahermes"), home)})
        return
...
def cmd_setup(args: argparse.Namespace) -> None:
    ...
    if args.reset or not state_path.exists():
        state = default_state(catalog, line_id=line, machine_id=machine, display_name=args.display_name or "TamaHermes")
    else:
    ...
    if resolve_target(args) == "hermes":
        ...
        response["hermesPet"] = select_hermes_pet(installed_state.get("petId", "tamahermes"), home)
    ...
    if args.json:
        ...
        if refresh["refreshed"]:
            ...
            print(f"installed: already up to date in {home / 'pets' / installed_state.get('petId', 'tamahermes')}")
...
def cmd_preview(args: argparse.Namespace) -> None:
    ...
    print(f"TamaHermes preview running at {url}", flush=True)
    try:
...
def cmd_bridge(args: argparse.Namespace) -> None:
    _root, home, state_path, catalog = context(args)
    build_dir = Path(args.build_dir).expanduser().resolve() if getattr(args, "build_dir", None) else home / "tamahermes" / "build"
    catalog_dir = str(catalog.root) if getattr(args, "catalog_dir", None) else None
    for line_number, line in iter_jsonl_source(args.input, follow=args.follow, poll_interval=args.poll_interval, stdin=sys.stdin):
        try:
            ...
            result = {"ok": False, "schema": "tamahermes.bridge.event.v1", "line": line_number, "error": str(exc)}
            emit_bridge_result(result, args.json)
            if args.strict:
...
def cmd_petdex(args: argparse.Namespace) -> None:
    ...
    if petdex_home is None:
        raise SystemExit(
            "no Petdex home given: pass --petdex-home ~/.petdex (or set TAMAHERMES_PETDEX_HOME)"
        )
...
def cmd_hermes_hook(args: argparse.Namespace) -> None:
    ...
    hook_state_path = (
        ...
        else home / "tamahermes" / "hermes-hook-state.json"
    )
    ...
    if args.json:
        ...
        print(f"tamahermes: {', '.join(report['events'])}", flush=True)
...
def cmd_codex_events(args: argparse.Namespace) -> None:
    ...
    cursor_path = Path(args.cursor).expanduser().resolve() if args.cursor else home / "tamahermes" / "codex-events-cursor.json"
...
def cmd_import(args: argparse.Namespace) -> None:
    ...
    restored_profiles = 0
    if args.profiles:
        profile_dir = Path(args.profile_dir).expanduser().resolve() if args.profile_dir else root / "tamahermes_gen" / "profiles"
...
def cmd_render_catalog(args: argparse.Namespace) -> None:
    root = resolve_repo_root(args.repo_root)
    script = root / "tamahermes_gen" / "scripts" / "render_catalog.py"
...
def cmd_doctor(args: argparse.Namespace) -> None:
    _root, home, state_path, catalog = context(args)
    state = load_state(state_path, catalog, line_id=args.line, machine_id=args.machine)
    build_dir = home / "tamahermes" / "doctor"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tamahermes", description="TamaHermes runtime and Codex custom pet installer.")
    add_common(parser)
    sub = parser.add_subparsers(dest="command", required=True)

    init = sub.add_parser("init", help="Create or reset the TamaHermes state ledger.")
    init.add_argument("--line", default="toast")
    init.add_argument("--machine", default="aurora")
    init.add_argument("--display-name", default="TamaHermes")
    ...
    setup = sub.add_parser("setup", help="Install or switch TamaHermes for Codex App in one step.")
    ...
    hermes_hook.add_argument("--hook-state", help="Hook bookkeeping path. Defaults to <home>/tamahermes/hermes-hook-state.json.")
    ...
    codex_events.add_argument("--cursor", help="Cursor path. Defaults to <codex-home>/tamahermes/codex-events-cursor.json.")
    ...
    generate = sub.add_parser("generate-profile", help="Validate a Codex-agent-authored tamahermes_gen profile.")
    ...
    render = sub.add_parser("render-catalog", help="Render a M2.1-compatible catalog with tamahermes_gen.")
    ...
    export.add_argument("--no-profiles", action="store_true", help="Skip tamahermes_gen profile JSON in the export bundle.")
    export.set_defaults(func=cmd_export)

    import_bundle = sub.add_parser("import", help="Import a TamaHermes export bundle into the state ledger.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        print(f"tamahermes: {exc}", file=sys.stderr)
        raise SystemExit(1) from exc
