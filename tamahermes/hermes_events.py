"""Hermes Agent adapter for the TamaHermes growth bridge.

TamaHermes's Codex integration has two halves: a *pet package* (``pet.json`` +
``spritesheet.webp``) and an *event feed* that grows the pet. Codex feeds the
ledger through its ``PostToolUse`` plugin hook (``plugins/tamahermes``); Hermes
...
hook payload into zero or more ``tamahermes.bridge.event.v1`` records, and never
...
The event taxonomoy matches ``tamahermes/state.py`` exactly, so Hermes activity
...
HERMES_HOOK_SCHEMA = "tamahermes.hermes_hook.v1"
...
def bridge_record(event: str, normalized: dict[str, Any], meta: dict[str, Any] | None = None) -> dict[str, Any]:
    """Build one ``tamahermes.bridge.event.v1`` record from a Hermes payload."""
    turn = turn_key(normalized)
...
def default_petdex_home(home: Path | None = None) -> Path | None:
    ...
    ``TAMAHERMES_PETDEX_HOME`` is set, or the installer recorded a pointer beside
    the ledger (``<home>/tamahermes/petdex-home``).
    """
    ...
    marker = base / "tamahermes" / "petdex-home"
    try:
        if marker.is_file():
...
def apply_hermes_hook(
    ...
    *,
    ...
    ledger's own selection so an existing TamaHermes is never re-hatched.
    """
