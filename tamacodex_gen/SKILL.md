---
name: tamacodex_gen
description: Generate complete Tamacodex milestone pawn/tamago asset catalogs from one or more short character prompts or profile JSON files, including lifecycle evolution forms, manifests, previews, and QA reports.
---

# Tamacodex Gen

Use this skill when asked to create or update a Tamacodex character catalog from
a concept such as “generate a Tamacodex named Ducky inspired by a duck”.
...
```bash
python tamacodex_gen/scripts/render_catalog.py \
  --milestone M2.1 \
  --asset-version m2.1 \
  --output-dir milestones/M2.1 \
  --profile tamacodex_gen/profiles/toast.json \
  --profile tamacodex_gen/profiles/mais.json
```

Parse a simple prompt into a profile:

```bash
python tamacodex_gen/scripts/prompt_to_profile.py \
  --prompt "基于 tamacodex_gen 下的内容，生成一个叫做 Ducky 的 tamacodex，它的灵感来自鸭子" \
  --brief-output /tmp/ducky-profile-brief.md \
  --output tamacodex_gen/profiles/ducky.json
```
...
```bash
python tamacodex_gen/scripts/prompt_to_profile.py \
  --input tamacodex_gen/profiles/ducky.json \
  --output tamacodex_gen/profiles/ducky.json
```
