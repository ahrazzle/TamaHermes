#!/usr/bin/env python3
"""Validate a Codex-agent-authored Tamacodex profile JSON.
...
def slugify(value: str) -> str:
    ...
    return slug or "tamacodex"


def titleize(slug: str) -> str:
    return " ".join(part.capitalize() for part in slug.split("_") if part) or "Tamacodex"
...
def normalize_profile(data: dict[str, Any]) -> dict[str, Any]:
    profile_id = slugify(str(data.get("id") or data.get("displayName") or "tamacodex"))
    ...
    description = str(data.get("description") or f"A {inspiration}-inspired Tamacodex companion.").strip()
...
def agent_brief(prompt: str, destination: str | None) -> str:
    ...
    return f"""# Tamacodex Profile Agent Brief

Create one Tamacodex profile JSON object for this user prompt:
...
```bash
python tamacodex_gen/scripts/prompt_to_profile.py --input {destination_line} --output {destination_line}
```
"""


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a Codex-agent-authored Tamacodex profile JSON.")
