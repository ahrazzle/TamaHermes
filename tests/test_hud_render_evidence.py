...
class RenderEvidenceTests(unittest.TestCase):
    ...
    def test_generator_writes_every_variant_and_a_clean_manifest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(geometry["panel"]["width"], 376)
            self.assertEqual(geometry["panel"]["height"], 226)
            self.assertEqual(geometry["panel"]["floors"], [120, 80])
            self.assertEqual(geometry["lcdInset"], {"left": 58, "right": 58, "top": 30, "bottom": 18})
            self.assertNotIn("collapsed", geometry)
